...
/**
 * 2. MOTOR DE SIMULACIÓN "MYTHECA SINGULARITY" CON SOPORTE DE MAPA
 */
class MythicaSingularityEngine extends Room<WorldState> {
    ...
    onCreate() {
        ...
        this.onMessage("mover", (client, data) => {
            ...
            // Fisica vectorial para evitar teletransportación
        ...
        // CORRECCIÓN APLICADA AQUÍ: Se usa _client para evitar el error de "value never read"
        this.onMessage("change_tile", (_client, data) => {
            const index = data.y * this.state.width + data.x;
            if (this.state.map[index] !== undefined) {
                this.state.map[index] = data.tile;
            }
        });
    }
...
server.listen(PORT, "0.0.0.0", () => {
    console.log("========================================");
    console.log(` NÚCLEO MYTHECA: SINGULARIDAD ACTIVA `);
